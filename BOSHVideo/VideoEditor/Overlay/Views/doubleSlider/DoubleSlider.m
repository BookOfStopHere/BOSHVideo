...
- (id) initWithFrame:(CGRect)aFrame barHeight:(float)height
{
    ...
    if (self)
	{
        self.minValue = 0;
        self.maxValue = 1;
        valueSpan = self.maxValue - self.minValue;
		...
		self.minHandle = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"handle.png"] highlightedImage:[UIImage imageNamed:@"handle_highlight.png"]];
		self.minHandle.center = CGPointMake(sliderBarWidth * 0.2, sliderBarHeight * 0.5);
		[self addSubview:self.minHandle];
		
		self.maxHandle = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"handle.png"] highlightedImage:[UIImage imageNamed:@"handle_highlight.png"]];
...
{
    _minValue = minValue;
    NSAssert(_minValue < _maxValue, @"最小值必须小于最大值");
    valueSpan = self.maxValue - self.minValue;
    [self updateValues];
...
-(void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if(self.sliderChangedHandler) self.sliderChangedHandler();
}
...
- (void)updateValues
{
	self.minSelectedValue = self.minValue + self.minHandle.center.x / sliderBarWidth * valueSpan;
    //snap to min value
    if (self.minSelectedValue < self.minValue + kBoundaryValueThreshold * valueSpan) self.minSelectedValue = self.minValue;
        
    self.maxSelectedValue = self.minValue + self.maxHandle.center.x / sliderBarWidth * valueSpan;
    //snap to max value
    if (self.maxSelectedValue > self.maxValue - kBoundaryValueThreshold * valueSpan) self.maxSelectedValue = self.maxValue;
}
