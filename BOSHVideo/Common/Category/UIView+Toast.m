...
static const CGFloat CSToastHorizontalPadding   = 25.0;
static const CGFloat CSToastVerticalPadding     = 10.0; // 10 as Default
static const CGFloat CSToastVerticalBottomMargin    = 60.0; // 60. as Default
...
// chat message
//static const CGFloat CSChatToastHorizontalPadding   = 15.0;
//static const CGFloat CSChatToastFontSize            = 13.0;

// shadow appearance
static const CGFloat CSToastShadowOpacity       = 0.8;
static const CGFloat CSToastShadowRadius        = 6.0;//6.0
...
NSString * const CSToastPositionUserCar         = @"usercar";
NSString * const CSToastPositionKeyboard        = @"keyboard";
NSString * const CSToaskPositonScreenCenter     = @"ScreenCenter";
...
+ (void)keyboardWillChangeFrame:(NSNotification *)notification {
    ...
    CGFloat toHeight = endFrame.size.height;
    if (endFrame.origin.y == [[UIScreen mainScreen] bounds].size.height) {
        toHeight = 0;
    }
    keyboardHeight = toHeight;
}
...
- (void)setToastView:(UIView *)tv
{
    objc_setAssociatedObject(self, &kDHStyleKey, tv, OBJC_ASSOCIATION_RETAIN);
}
...
- (void)makeChatRoomToast:(NSString *)message {
    UIView *toast = [self viewForChatMessage:message];
    [self showToast:toast duration:CSToastDefaultDuration position:@"center"];
}
...
- (void)makeChatRoomToastUserCar:(NSString *)message {
    UIView *toast = [self viewForChatMessageWithBackgroundColor:[UIColor blackColor] textColor:[UIColor whiteColor] andMessage:message];
    [self showToast:toast duration:CSToastDefaultDuration position:CSToastPositionUserCar];
}

- (void)makeChatRoomToastKeyboard:(NSString *)message {
    UIView *toast = [self viewForChatMessageWithBackgroundColor:[UIColor blackColor] textColor:[UIColor whiteColor] andMessage:message];
    [self showToast:toast duration:CSToastDefaultDuration position:CSToastPositionKeyboard];
}

- (void)makeChatRoomToastOnTop:(NSString *)message {
    UIView *toast = [self viewForChatMessage:message];
    [self showToast:toast duration:CSToastDefaultDuration position:@"top"];
}
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        } else if ([point caseInsensitiveCompare:CSToastPositionUserCar] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height - 110);
        } else if ([point caseInsensitiveCompare:CSToastPositionKeyboard] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height - 300);
        } else if ([point caseInsensitiveCompare:CSToaskPositonScreenCenter] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width/2, self.frame.size.height / 2);
        }
    ...
    // default to bottom
    return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalBottomMargin - keyboardHeight);
}
...
//    wrapperView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:CSToastOpacity];
    wrapperView.backgroundColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:CSToastOpacity];
...
- (UIView *)viewForChatMessage:(NSString *)message {
...
//    wrapperView.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:CSToastOpacity];
    ...
    CGFloat wrapperWidth = messageLeft + messageWidth + CSToastHorizontalPadding;
...
- (UIView *)viewForChatMessageWithBackgroundColor:(UIColor *)bkgColor textColor:(UIColor *)textColor andMessage:(NSString *)message {
    if(message == nil || [message isKindOfClass:[NSString class]]==NO) {
        return nil;
    }
    UILabel *messageLabel = nil;
...
//    wrapperView.backgroundColor = [bkgColor colorWithAlphaComponent:CSToastOpacity];
    wrapperView.backgroundColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:CSToastOpacity];
